...
def create_synthetic_multi_source_dataset() -> Dataset:
    ...
    ds.add_orderbook_series("BTCUSDT", "1h", orderbook_imbalance)
    ...
    ds.add_series("BTCUSDT", "1h", orderbook_spread, source="orderbook")
...
class TestMultiSourceExpressions:
    ...
    def test_multi_source_comparison(self):
        ...
        expr = compile_expression(
            "sma(BTC.price, period=20) > sma(BTC.trades.volume, period=20) / 100"
        )
        result = expr.run(ds)
    ...
    def test_plan_requirements_multi_source(self):
        ...
        trades_reqs = [
            req for req in plan.requirements.data_requirements
            if req.source == "trades"
        ]
    ...
    def test_nested_expression_multi_source(self):
        ...
        expr = compile_expression(
            "sma(BTC.price + BTC.trades.volume / 1000, period=10)"
        )
        result = expr.run(ds)
    ...
    def test_time_shift_multi_source(self):
        ...
        expr = compile_expression("BTC.trades.volume > BTC.trades.volume.24h_ago")
        result = expr.run(ds)
