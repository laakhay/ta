...
def _select_field(ctx: SeriesContext, field: str) -> Series[Price]:
    ...
    if field_lower in ("hlc3", "typical_price"):
        if "high" not in ctx.available_series or "low" not in ctx.available_series or "close" not in ctx.available_series:
            raise ValueError(f"SeriesContext missing required fields for '{field}': need 'high', 'low', 'close'")
        high = getattr(ctx, "high")
        low = getattr(ctx, "low")
        close = getattr(ctx, "close")
        return (high + low + close) / Decimal(3)
    
    # ohlc4 - Average Price: (open + high + low + close) / 4
    if field_lower in ("ohlc4", "weighted_close"):
        if "open" not in ctx.available_series or "high" not in ctx.available_series or "low" not in ctx.available_series or "close" not in ctx.available_series:
            raise ValueError(f"SeriesContext missing required fields for '{field}': need 'open', 'high', 'low', 'close'")
        open_series = getattr(ctx, "open")
        high = getattr(ctx, "high")
        low = getattr(ctx, "low")
        close = getattr(ctx, "close")
    ...
    if field_lower == "upper_wick":
        if "high" not in ctx.available_series or "open" not in ctx.available_series or "close" not in ctx.available_series:
    ...
    if field_lower == "lower_wick":
        if "open" not in ctx.available_series or "close" not in ctx.available_series or "low" not in ctx.available_series:
