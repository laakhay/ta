...
def in_channel(
    ...
    if len(price_series) == 0:
        return Series[bool](
            timestamps=(), values=(), symbol=price_series.symbol, timeframe=price_series.timeframe
        )
    ...
    result_values = tuple(
        (price_aligned.values[i] >= lower_aligned.values[i])
        and (price_aligned.values[i] <= upper_aligned.values[i])
...
def out(
    ...
    result_values = tuple(
        (price_aligned.values[i] > upper_aligned.values[i])
        or (price_aligned.values[i] < lower_aligned.values[i])
        for i in range(len(price_aligned))
    )
