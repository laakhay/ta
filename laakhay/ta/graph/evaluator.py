...
class Evaluator:
    ...
    def _evaluate_dataset(self, expr, plan: PlanResult, dataset: Dataset) -> dict[tuple[str, str, str], Series[Any]]:
        ...
        for symbol, timeframe in unique_keys:
            ...
            node_cache_key = (plan.graph_hash, plan.graph.root_id, alignment_key, symbol, timeframe)
    ...
    def _evaluate_graph(self, plan: PlanResult, context: dict[str, Any], symbol: str = None, timeframe: str = None) -> Any:
        graph = plan.graph
        ...
        for node_id in order:
            ...
            cache_key = (plan.graph_hash, node_id, alignment.cache_key(), symbol, timeframe) if symbol is not None and timeframe is not None else None
